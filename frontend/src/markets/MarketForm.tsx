...
export function MarketForm({ market, companies, authorities = EMPTY_AUTHORITIES, consortia = EMPTY_CONSORTIA, onSaved, onConsortiumCreated }: Props) {
  ...
  return <form className="card company-form" onSubmit={submit}>
    ...
    {!market && <><fieldset><legend>Type de titulaire</legend><label><input type="radio" checked={values.holder_type === 'SOLE_COMPANY'} onChange={() => update('holder_type', 'SOLE_COMPANY')} /> Société</label><label><input type="radio" checked={values.holder_type === 'CONSORTIUM'} onChange={() => update('holder_type', 'CONSORTIUM')} /> Groupement</label></fieldset><label>Société gestionnaire<select aria-label="Société gestionnaire" value={values.company} onChange={(e) => update('company', e.target.value)} required><option value="">Sélectionner une société</option>{editableCompanies.map((company) => <option key={company.id} value={company.id}>{company.raison_sociale}</option>)}</select><small className="muted">Société de votre espace qui gère ce marché dans RevisionPrix.</small>{errors.company && <small className="field-error">{errors.company.join(' ')}</small>}</label></>}
    ...
    {market && <label>Société gestionnaire<input value={market.company_detail.raison_sociale} readOnly /> <small className="muted">Société de votre espace qui gère ce marché dans RevisionPrix.</small></label>}
