...
export default function App() {
  ...
  return <Routes><Route path="/login" element={user ? <Navigate to="/app/companies" replace /> : <LoginPage onLogin={async () => setUser(await currentUser())} />} /><Route element={<Protected user={user} />}><Route element={<Layout user={user!} onLogout={async () => { await logout(); setUser(null) }} />}><Route path="/app/companies" element={<CompaniesPage />} /><Route path="/app/companies/new" element={<CompanyCreatePage />} /><Route path="/app/companies/:id" element={<CompanyDetailPage />} /><Route path="/app/companies/:id/edit" element={<CompanyDetailPage />} /></Route></Route><Route path="*" element={<Navigate to={user ? '/app/companies' : '/login'} replace />} /></Routes>
}
