...
export function CompanyDetailPage() {
  ...
  return <section><div className="page-heading"><div><p className="eyebrow">SOCIÉTÉ</p><h1>{company.raison_sociale}</h1></div><div className="inline-actions">{canEdit && <button className="secondary" onClick={() => setEditing(true)}>Modifier</button>}<button className="link-button" onClick={() => navigate('/app/companies')}>Retour</button></div></div><div className="card detail-grid">{[['Forme juridique', company.forme_juridique], ['Capital social', formatMoney(company.capital_social)], ['ICE', company.ice || '—'], ['IF', company.if_fiscal || '—'], ['RC', company.rc || '—'], ['CNSS', company.cnss || '—'], ['Adresse', company.adresse_complete || '—'], ['Ville', company.ville || '—'], ['Téléphone', company.telephone || '—'], ['E-mail', company.email || '—'], ['Site web', company.site_web || '—'], ['Représentant', `${company.representant_prenom} ${company.representant_nom}`.trim() || '—'], ['Fonction', company.representant_fonction || '—']].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</div></section>
}
